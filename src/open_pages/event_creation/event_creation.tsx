...
import { Coordinates, ErrorBody, EventCreationReq, PopularTags } from "../../utils/interfaces";
...
export default function EventCreation(): JSX.Element {
  ...
  const [listOfPopularTagsForChecking, setListOfPopularTagsForChecking] = useState<string[]>([]);
  ...
  const [isSuccessfulAlertVisible, setSuccessfulAlertVisibility] = useState<boolean>(false);
  const [isMapVisible, setMapVisibility] = useState<boolean>(false);
  const [locationName, setLocationName] = useState<string>("Where?");

  const [coordinates, setCoordinates] = useState<Coordinates>({ lat: 54.843417, lng: 83.090425});
  ...
  const onDateClose = (date: Moment | string) => {
    if(typeof date === 'string'){
      ...
      if(date.isBefore(today)){
        enableAlert("This date is from past. Please pick valid date ;)","warning");
        setDateCorrectness(false);
      } else {
      ...
      getPopularTags()
        .then((resp) => {
          ...
          if (code === 200){
            ...
            tags = tags.filter((tag, index, arr) => !listOfPopularTags.includes(tag) && !listOfPickedTags.includes(tag));
  ...
  const createNewTag = () => {
    if (newTag.length===0){
      ...
      enableAlert("This tag already exists in popular tags, you can pick it from popular list", "warning")
    }
    else{
  ...
  const onEventCreation = () => {
    if (!isDateCorrect){
      ...
      enableAlert("Fields Name, Description, Location(Where?) are mandatory. Please fulfill them all!", "warning");
    else
      {
        let login="";
        whoAmI().then(
        ...
        createEvent(eventData).then(
          (result) =>{
            if(result === true){
              ...
              enableAlert("Event has not been created due to: " + result as string,"danger");
            }
          }
        );
      }
  ...
  return (
      <Container className={styles.mainContainer}>
        ...
        >
          ...
          <Button variant={"dark"} onClick={onEventHasBeenCreated}>Cool!</Button>
        </Alert>
        ...
        <Container className={styles.eventCreationContainer}>

          {
            ...
            <Form.Group className={styles.tagEditForm}>
              ...
              <div className={styles.tagEditButtonsContainer}>
                <Button
                  variant={"success"}
                  onClick={saveTagNameChanges}>
                  Save
                </Button>
                <Button
                  variant={"danger"} className={styles.cb}
          ...
          <Col className={styles.eventCreationForm} md={{span: 4}}>
                  ...
                  listOfPickedTags.length>0?
                    <>
                      ...
                      <Col lg={{span: 1, offset: 10}}>
                        <OverlayTrigger trigger={"click"} placement={"right"} overlay={popover} show={isTagAddingVisible}>
              ...
              <Row style={{marginTop: 10}}>
                <Col md={{span: 3}}>
                  <Button variant="success" onClick={onEventCreation}>Create</Button>
                </Col>
                <Col md={{span: 3}}>
